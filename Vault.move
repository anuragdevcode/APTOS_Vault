module minivault::vault {
    ...
    public entry fun deposit<CoinType>(vault_address: address, account: &signer, amount: u64) acquires Vault, Fuse {
    ...
    public entry fun withdraw<CoinType>(vault_address: address, account: &signer, amount: u64) acquires Vault, Fuse {
        assert!(exists_vault<CoinType>(vault_address), ERR_VAULT_NOT_EXISTS);
    ...
    fun withdraw_internal<CoinType>(vault_address: address, amount: u64): Coin<CoinType> acquires Vault, Fuse {
        ...
        coin::extract(&mut borrow_global_mut<Vault<CoinType>>(vault_address).coin, amount);
    }
    ...
    public fun op_fail_when_paused(admin: &signer, user: &signer) acquires Vault, Fuse {
        ...
        deposit<FakeCoin>(signer::address_of(user), user, 10000);
    }
